...
new_signature_to_bytes(byte_t *enc, const new_signature_t *sig)
{
    ...
    enc = ec_basis_to_bytes(enc, &sig->B_aux);
    encode_digits(enc, sig->q, TORSION_2POWER_BYTES);
...
new_signature_from_bytes(new_signature_t *sig, const byte_t *enc)
{
    ...
    enc = ec_basis_from_bytes(&sig->B_aux, enc);
    decode_digits(sig->q, enc, TORSION_2POWER_BYTES, NWORDS_ORDER);
    enc += TORSION_2POWER_BYTES;
