...
def EquivalentSpecialIdeal(I, d1, d2):
    ...
    # Step 0: compute M_Gram = B*D*B^T / N(I)
    ...
    # Step 2: compute Q(w) = 0 (mod d1), and satisfies Q(u + lambda*w) = d2 (mod d1)
    # Q(u + lambda* w) = (u + lambda* w)*M_Gram*(u + lambda* w)^T
    #                 = u*M_Gram*u^T + lambda*u*(M+M^T)*w^T + lambda^2*(w*M*w^T)
    ...
    # Lambda_w = {v in Z^4 | v = lambda*w (mod d1), lambda in Z}
    # for any v in Lambda_w, v = [v1, v2, v3, v4] = lambda*[w1, w2, w3, w4] + d1*[k1, k2, k3, k4], ki in Z.
    # abstract basis of lambda_w under the basis of I = [alpha1, alpha2, alpha3, alpha4]: 
    # [d1,  0,  0,  0]
    # [ 0, d1,  0,  0]
    # [ 0,  0, d1,  0]
    # [ 0,  0,  0, d1]
