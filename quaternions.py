from sage.all import (
    randint, mod, valuation, kronecker, gcd, ZZ, QQ, GF, PolynomialRing, is_prime, Matrix, matrix, round,
    vector, Zmod, isqrt, ceil, sqrt, diagonal_matrix, lcm
)
...
def EquivalentSpecialIdeal(I, d1, d2):

    # Step 0: compute M_Gram = B*D*B^T / N(I0)
    B_lideal = matrix(QQ, I.basis_matrix())
    N_I = I.norm()
    D = diagonal_matrix(QQ, [1, 1, params.p, params.p])
    
    M_Gram = (B_lideal * D * B_lideal.transpose()) / N_I

    # Step 1: compute Q(u) = d2 (mod d1)
    # for x = [x1, x2, x3, x4](xi in Z) in I, Q(x) = x*M_Gram*x^T = Nrd(x) / Nrd(I)
    F = GF(d1)
    M = matrix(F, M_Gram)
    P = PolynomialRing(F, 'x')
    x = P.gen()

    # for u = [u1, u2, u3, u4], randomly sample u2, u3, u4, then solve a quadratic equation in u1.
    # Q(u) = u*M_Gram*u^T = d2 (mod d1) 
    u = None
    while u is None:
        u_vals = [x, F.random_element(), F.random_element(), F.random_element()]
        v = vector(P, u_vals)
        eq = v * M * v - F(d2) 
        roots = eq.roots()
        if roots:
            u = vector(F, [poly(roots[0][0]) for poly in v])

    # Step 2: compute Q(w) = 0 (mod d1), and satisfies Q(u + lamba*w) = d2 (mod d1)
    # Q(u + lamba* w) = (u + lamba* w)*M_Gram*(u + lamba* w)^T
    #                 = u*M_Gram*u^T + lamba*u*(M+M^T)*w^T + lamba^2*(w*M*w^T)
    # it is required B(u, w) = u*(M+M^T)*w^T = 0 (mod d1)
    #                Q(w) = w*M*w^T = 0 (mod d1)

    M_sym = M + M.transpose()
    C = u * M_sym

    pivot = next((i for i in range(4) if C[i] != 0), -1)
    if pivot == -1: raise ValueError("Degeneracy condition: u is located in the root space.")
        
    # C = u*(M+M^T) = [C1, C2, C3, C4]
    # w1 = =C1^(-1)*(C2*w2 + C3*w3 +C4*w4) (mod d1)
    # for w = [w1, w2, w3, w4], randomly sample w3, w4, then solve a quadratic equation in w2.
    inv_C = 1 / C[pivot]
    w = None
    while w is None:
        w_vals = [F.random_element() for _ in range(4)]
        w_vals[pivot] = 0
        w_vals[(pivot + 1) % 4] = x 
        w_vals[pivot] = -inv_C * sum(C[i] * w_vals[i] for i in range(4) if i != pivot)
        
        vec_w = vector(P, w_vals)
        eq = vec_w * M * vec_w
        roots = eq.roots()
        if roots:
            w_cand = vector(F, [poly(roots[0][0]) for poly in vec_w])
            if w_cand != 0: w = w_cand

    # Step 3: construct sublattice basis
    # Lamba_w = {v in Z^4 | v = lamba*w (mod d1), lamba in Z}
    # for any v in Lamba_w, v = [v1, v2, v3, v4] = lamba*[w1, w2, w3, w4] + d1*[k1, k2, k3, k4], ki in Z.
    # abstract basis of Lamba_w under the basis of I = [alpha1, alpha2, alpha3, alpha4]: 
    # [d1,  0,  0,  0]
    # [ 0, d1,  0,  0]
    # [ 0,  0, d1,  0]
    # [ 0,  0,  0, d1]
    # [w1, w2, w3, w4]
    B_sublat = [[0,0,0,0] for _ in range(4)]
    for i in range(4): B_sublat[i][i] = d1
    B_sublat.append([ZZ(val) for val in w])
    B_sublat = matrix(ZZ, B_sublat)
    # Hermite Normal Form
    B_sublat = matrix(ZZ, [r for r in B_sublat.hermite_form().rows() if not r.is_zero()])

    # Step 4: perform LLL-reduction under the quaternion norm
    # convert to the real basis under [1, i, j, k] and stretch to Euclidean space
    S = ZZ(params.p).isqrt()
    D_embed = diagonal_matrix(QQ, [1, 1, S, S])
    B_sublat_embed = B_sublat * B_lideal * D_embed
    # LLL reduction is more efficient for integer matrices.
    denom_sublat_embed = lcm([x.denominator() for row in B_sublat_embed for x in row])
    B_sublat_embed_ZZ = matrix(ZZ, B_sublat_embed * denom_sublat_embed)
    B_sublat_embed_ZZ_lll = B_sublat_embed_ZZ.LLL()

    T_sublat = matrix(ZZ, B_sublat_embed_ZZ_lll * B_sublat_embed_ZZ.inverse())
    B_sublat_lll = T_sublat * B_sublat

    # Step 5: solve the CVP using exact Babai rounding and compute the equivalent left-ideal J with n(J) = k*d1 + d2
    u_ZZ = vector(ZZ, [ZZ(val) for val in u])
    t_QQ = -vector(QQ, u_ZZ)
    # exact Babai rounding
    c_exact = t_QQ * matrix(QQ, B_sublat_lll).inverse()
    c_round = vector(ZZ, [val.round() for val in c_exact])
    v_ZZ = c_round * B_sublat_lll
    # n(J) = n(alpha) / n(I)
    alpha_ZZ = u_ZZ + v_ZZ
    alpha_QQ = vector(QQ, alpha_ZZ)
    alpha_norm_QQ = alpha_QQ * M_Gram * alpha_QQ
    # check if n(J) = k*d1 + d2
    if alpha_norm_QQ.denominator() != 1: 
        raise ValueError("n(J) include the denominator!")
    if (alpha_norm_QQ - d2) % d1 != 0: 
        raise ValueError("The congruence condition is not met!")
    k = (alpha_norm_QQ - d2) // d1

    # J = I * alpha_conjugate / n(I)
    alpha = alpha_ZZ * B_lideal
    alpha = params.B(list(alpha))
    J = I * alpha.conjugate() * (1/N_I) 
